...
class ShowOff < Sinatra::Application
  ...
  settings.assets.append_path 'batman'
  settings.assets.append_path settings.pres_dir
  settings.assets.append_path File.join(settings.pres_dir, 'resources')
  ...
  helpers do
    ...
    def js_files
      Dir.glob("#{settings.pres_dir}/*.{js,coffee}").map { |path| File.basename(path) }
    end
  ...
  get %r{(?:image|file|assets)/(.*)} do
    path = params[:captures].first
    # settings.assets.append_path settings.pres_dir
    # full_path = File.join(settings.pres_dir, path)
    asset = settings.assets[path]
